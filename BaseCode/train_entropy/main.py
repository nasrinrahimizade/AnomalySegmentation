...
class MaxEntropyLoss(torch.nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        entropy = entropy / (torch.log(torch.tensor(probs.shape[1], dtype=torch.float32)) + 1e-10)
        # print(entropy.shape) #[6,64,128]
        
        # entropy = torch.div(torch.sum(-probs * torch.log(probs), dim=1), torch.log(torch.tensor(probs.shape[1]))+ 1e-10).float()
        # total_loss = self.loss(entropy,targets)
        # print(entropy.shape) #[6,64,128]
        # print(entropy)
        main_loss =self.loss(outputs,targets)
        alpha = 0.1
    # #     if(torch.mean(entropy)):
    # #         print(torch.mean(entropy))
    # #     else:
    # #         print(entropy)
    # # # Total loss with entropy regularization
        total_loss = main_loss + alpha *torch.mean(entropy)
        # print(main_loss)
    #     print(total_loss)
        return total_loss
...
#         probs = torch.nn.functional.softmax(outputs, dim=1)
#         log_softmax = torch.log(probs + 1e-10)
#         # print(probs.shape)
#         # print(log_softmax.shape)
#         entropy = -torch.sum(probs * log_softmax, dim=1)
    
#         # print(entropy.shape)
#         # print(targets.shape)
#         loss = dice_loss(entropy, targets)
#         # print(loss.item())
#         return loss
