...
class IsoMaxPlusLossSecondPart(nn.Module):
    ...
    def forward(self, logits, targets, debug=False):
        ...
        # print("h")
        # print(probabilities_for_training.shape) #[6,20,64,128]
        # print(range(distances.size(0))) #range(0,6)
        # print()
        batch_size, num_classes, height, width = probabilities_for_training.shape

        batch_indices = torch.arange(batch_size).unsqueeze(1).unsqueeze(2).expand(batch_size, height, width)

        probabilities_at_targets = probabilities_for_training[batch_indices, targets]
