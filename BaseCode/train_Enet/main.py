...
def train(args, model):
    ...
    for epoch in range(start_epoch, args.num_epochs+1):
        ...
        for step, (images, labels) in enumerate(loader):
            ...
            # print(inputs.shape) #should be =>torch.Size([6, 3, 512, 1024])
            
            outputs = model(inputs)


            # print(outputs.shape) #should be  => torch.Size([6, 20, 64, 128])
            # print(targets.shape) # [6,1,64,128]
...
def main(args):
    ...
    model = model_file.enet(NUM_CLASSES)###########Here we are importing the model##########
    ...
    #     model = model_file.enet(NUM_CLASSES, encoder=pretrainedEnc)  #Add decoder to encoder
    #     if args.cuda:
    #         model = torch.nn.DataParallel(model).cuda()
